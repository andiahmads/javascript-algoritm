...
function capitalize(str) {
  const worldCase = str.split(" ");
  const arrayOfworldCase = [];

  for(let i = 0; i<worldCase.length; i++) {
    const world = worldCase[i];

    arrayOfworldCase.push(world[0].toUpperCase() + world.slice(1).toLowerCase())
  }
  return arrayOfworldCase.join(" ")
}
